...
public final class IFDService implements IFD {
	 ...
	 * 
	 * @see
	 * iso.std.iso_iec._24727.tech.schema.IFD#transmit(iso.std.iso_iec._24727
	 * .tech.schema.Transmit)
	 */
	@Override
	public iso.std.iso_iec._24727.tech.schema.TransmitResponse transmit(
		...
		for (final InputAPDUInfoType apdu : parameters.getInputAPDUInfo()) {
			...
			if (apduMsg != null && apduMsg.length > 0) {
				/*
				 * CLA = 0xFF has a special meaning as it is parsed by the
				 * terminal driver or the terminal firmware, but not by the
				 * card. So disallow it for requests from eID server side.
				 */
